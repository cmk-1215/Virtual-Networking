...
io.on("connect", (socket) => {
  ...
  socket.on("JOIN_EVENT", function (data) {
    socket.on("disconnect", function () {
      ...
      User.findOneAndDelete({ _id: data.user._id }).exec((err, user) => {
        if (err) return next(err);
      });
      //remove conversation from user

      Event.findById(data.eventId).exec((err, event) => {
        // search the users array for user id
      ...
      Conversation.find({ users: data.user._id }).exec((err, conversations) => {
        // search the users array for user id

        conversations.forEach((conversation) => {
          const userIndex = conversation.users.findIndex(
            (user) => user == data.user._id
          );
...
//   eventName: "Project Shift Demo Day",
// });
